...
// authenticated cloud models. Like withRedirectedDefaultTransport, callers
// must not run in parallel.
func newAuthenticatedCloudBackend(t *testing.T, addr string) *cloud.Client {
	t.Helper()
...
// Client's HTTP client falls back to, since it never sets its own
...
func TestCloudAuthenticate(t *testing.T) {
	...
	cases := []testCase{
		...
		},
		{
			...
			wantErrIs: models.ErrMissingAuthFile,
		},
		{
		...
		},
		{
			...
			wantErrIs: models.ErrEmailNotFound,
		},
	}
...
func TestCloudPostAPIOperation(t *testing.T) {
	...
	cases := []testCase{
		...
		},
		{
			...
			wantErrIs:    models.ErrRateLimited,
		},
		{
		...
		},
		{
			...
			wantErrIs:    models.ErrUnexpectedStatus,
		},
	}
...
func TestCloudSetGridCharging(t *testing.T) {
	...
	cases := []testCase{
		...
		},
		{
			...
			status: http.StatusTooManyRequests, wantErrIs: models.ErrRateLimited,
		},
		{
			name: "500 maps to ErrUnexpectedStatus", mode: true, wantDisallow: false,
			status: http.StatusInternalServerError, wantErrIs: models.ErrUnexpectedStatus,
		},
	}
...
func TestCloudSetGridExport(t *testing.T) {
	...
	cases := []testCase{
		...
		},
		{
			...
			status: http.StatusServiceUnavailable, wantErrIs: models.ErrRateLimited,
		},
		{
...
func TestCloudNetworkBackedBehavior(t *testing.T) {
	...
	// (pypowerwall_cloud.py:835-874), and returns models.ErrNotFound if
	...
	t.Run("Authenticate fails when no energy site is found", func(t *testing.T) {
		...
		err := c.Authenticate(t.Context())
		require.ErrorIs(t, err, models.ErrNoEnergySite)
	})
...
func TestCloudAuthenticateBootstrapsFromEnv(t *testing.T) {
	...
	t.Run("missing auth file and no env refresh token still fails", func(t *testing.T) {
		...
		err := c.Authenticate(t.Context())
		require.ErrorIs(t, err, models.ErrMissingAuthFile)
	})
}
