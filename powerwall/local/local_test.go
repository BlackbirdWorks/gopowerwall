...
// newBackend builds a Client pointed at host with the given auth mode and cache file.
func newBackend(host string, authMode models.AuthMode, cachefile string) *local.Client {
	return local.New(
...
func TestPollHTTPStatusHandling(t *testing.T) {
	...
	cases := []testCase{
		{name: "not found", status: http.StatusNotFound, wantErrIs: models.ErrNotFound},
		{name: "too many requests", status: http.StatusTooManyRequests, wantErrIs: models.ErrRateLimited},
		{name: "service unavailable", status: http.StatusServiceUnavailable, wantErrIs: models.ErrRateLimited},
		{name: "bad request", status: http.StatusBadRequest, wantErrIs: models.ErrUnexpectedStatus},
		{name: "internal server error", status: http.StatusInternalServerError, wantErrIs: models.ErrUnexpectedStatus},
	}
...
func TestPollNegativeCacheAndCooldown(t *testing.T) {
	...
	t.Run("429 activates cooldown blocking other endpoints", func(t *testing.T) {
		...
		_, err := b.Poll(t.Context(), "/api/status", false, false, false)
		require.ErrorIs(t, err, models.ErrRateLimited)
...
func TestPollLoginRetry(t *testing.T) {
	...
	t.Run("re-login fails and ErrLogin is returned", func(t *testing.T) {
		...
		_, err := b.Poll(t.Context(), "/api/status", false, false, false)
		require.ErrorIs(t, err, models.ErrLogin)
	})
}
...
// exercises both orderings against a single *Client so a fresh
...
func TestPollRawAndParsedCacheIsolation(t *testing.T) {
	...
	type testCase struct {
		run  func(t *testing.T, b *local.Client)
		name string
	}
